...
if (is_ajax()) {    
    if (isset($_POST["awardCategory"]) && !empty($_POST["awardCategory"])) { //Checks if category value exists
        ...
        if ($cat == "All" && $year == "All") {
            $sql = "SELECT poster_id, files_upload, title_of_poster, project_abstract, is_winner, award_winner_category, year FROM tbl_poster";
        } else if($cat == "All" && $year != "All"){
            $sql = "SELECT poster_id, files_upload, title_of_poster, project_abstract, is_winner, award_winner_category, year FROM tbl_poster WHERE year = $year";
            ...
            $sql = "SELECT poster_id, files_upload, title_of_poster, project_abstract, is_winner, award_winner_category, year FROM tbl_poster WHERE award_category LIKE '%$cat%'";
            // $sql = "SELECT poster_id, files_upload, title_of_poster, project_abstract, is_winner, award_winner_category, year FROM tbl_poster WHERE is_public = 1 AND year = $year AND award_category=''";
        } else {
