...
for($i = 0; $i < count($files_upload); $i++) {
    foreach($files_upload[$keys[$i]] as $key => $value) {
		...
		if (file_exists($filename)) {
			$uploadFiles .= "<br><a href=\"" . $url . '/posters/' . $value . '" target="_blank">' .  $value ."</a>";
			$all_file_name .= $value . ', ';
...
$mail->setFrom('events@rcc.uchicago.edu', 'Mind Bytes 2017');
...
$mail->Subject = "Mind Bytes 2017 Poster Submission Information : " . $fullname . " uploaded a Poster titled ". '"'. $title_of_poster .'"';

$mail->Body = "<b>Is Public:</b> " . $is_public . "<br><b>Full Name:</b> " . $fullname . "<br><b>Email:</b> " . $email . "<br><b>Academic Title:</b> " . $academic_title . "<br><b>Division/School:</b> " . $division . "<br><b>Department:</b> " . $department . "<br><b>RCC resources  used to conduct this research:</b> " . $rcc_resources . "<br><b>Principal Investigator:</b> " . $principal_investigator . "<br><b>Title of Poster/Project:</b> " . $title_of_poster . "<br><b>Project Abstract/Description:</b> " . $project_abstract . "<br><b>Award Category:</b> " . $award_category. "<br><b>Posters:</b> " . $uploadFiles;
...
$sql = "INSERT INTO tbl_poster (files_upload, is_public, fullname, email, academic_title, division, department, rcc_resources, principal_investigator, title_of_poster, project_abstract, award_category) 
	VALUES ('$all_file_name', $is_public, '$fullname', '$email', '$academic_title', '$division', '$department', '$rcc_resources', '$principal_investigator', '$title_of_poster', '$project_abstract', '$award_category')";
